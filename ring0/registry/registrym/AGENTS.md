This is the kernel module for the ardos registry written in rust.

# Build steps

Building is very easy, a cargo plugin called `cargo-nok` was created to easily build an rlib crate as a linux module, no need to worry about Kbuild or makefiles. `nok` in `cargo-nok` means `No-Kbuild` and it's an attempt to make a modern full rust build system for kernel modules, no make or C code involved.

```
$ cargo nok build
    Finished `dev` profile [unoptimized + debuginfo] target(s) in 0.01s
     Linking kernel object for `registrym`
  Processing module metadata for `registrym`
   Compiling module metadata for `registrym`
   Compiling common kernel module metadata
     Linking kernel module `registrym`
    Finished kernel module /home/tiago/ardos/ring0/registrym/target/target/debug/registrym.ko
```

It also has a helper for loading and unloading the kernel module, but it requires root previligies so you're not going to be able to use it directly.
```
$ cargo nok load
$ cargo nok reload
$ cargo nok unload
```
These are useful during development to test the module, although a bit dangerous if you use panic!() or .unwrap() a lot inside your code since rust panics will map to actual kernel panics.
