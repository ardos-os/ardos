{ ap2, self, pkg-config, lib, fetchgit, stdenv }:
...
ap2.buildArdosRustPackage {
  ...
  NIX_LDFLAGS = lib.concatMapStringsSep " " (dep: "-L${dep}/lib -rpath-link ${dep}/lib") [
    freetype2 libpng expat libdrm mtdev libevdev libwacom glib libffi libgudev glib.passthru.pcre2.out zlib
  ];
